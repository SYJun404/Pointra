import { MODIFIER_MAP } from "../data";

/** 单独渲染一个按键标签 */
export function KeyTag({ label }: { label: string }) {
    const display = MODIFIER_MAP[label] ?? label;
    return (
        <kbd className="inline-flex items-center justify-center min-w-[28px] h-6 px-1.5 text-[11px] font-semibold rounded-md bg-gray-100 border border-gray-200 text-gray-700 shadow-[inset_0_-1px_0_#d0d0d0]">
            {display}
        </kbd>
    );
}
